...
namespace FSH.Modules.Products.Features.v1.Commands.UpdateIssue;
    ...
    : ICommandHandler<UpdateIssueCommand>
{
    public async ValueTask<Unit> Handle(UpdateIssueCommand command, CancellationToken cancellationToken)
    {
        ...
        var tenantId = currentUser.GetTenant()
            ?? throw new InvalidOperationException("Unable to update issue: tenant context is required but not available.");
