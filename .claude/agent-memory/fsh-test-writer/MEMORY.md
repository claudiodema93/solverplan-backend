...
- csproj refs: `Modules.Products`, `Modules.Products.Contracts`, `Playground.Api`
- Packages: xUnit, Shouldly, NSubstitute, AutoFixture, Microsoft.AspNetCore.Mvc.Testing
...
- Instantiate `new XxxValidator()` directly in field: `private readonly XxxValidator _sut = new();`
...
## Namespaces
- Validator: `FSH.Modules.Products.Application.Features.v1.Commands.{Feature}`
- Command/Query (Contracts): `FSH.Modules.Products.Contracts.Application.Features.v1.Commands.{Feature}`
- Test namespace: `Products.Tests.Validators` / `Products.Tests.Integration`

## Key FSH Rules
- `ICommand<T>` returns `ValueTask<T>`, `ICommand` (no return) returns `ValueTask<Unit>`
- `IQuery<T>` returns `ValueTask<T>`
- Mediator (NOT MediatR) — `using Mediator;`
- Handlers use `ProductsDbContext` + `ICurrentUser` directly (no repository abstraction)
